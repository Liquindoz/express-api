pipeline {
  ...
  environment {
    SCANNER_HOME   = tool 'SonarScanner'   // keep if you use SonarQube
    HOST_PORT_TEST = '3031'                // staging host port
    HOST_PORT_PROD = '3032'                // production host port
    APP_PORT       = '3000'                // container/app port
  }
  ...
  stages {
      ...
      steps {
        script {
          ...
          sh """#!/bin/bash
            ...
            echo "[Deploy] Start STAGING host:${HOST_PORT_TEST} -> app:${APP_PORT}"
            docker run -d --name express-api-test -p ${HOST_PORT_TEST}:${APP_PORT} \
      ...
      steps {
        script {
          ...
          sh """#!/bin/bash
            ...
            echo "[Release] Clean previous PROD"
            docker rm -f express-api-prod || true

            echo "[Release] Start PRODUCTION host:${HOST_PORT_PROD} -> app:${APP_PORT}"
            docker run -d --name express-api-prod -p ${HOST_PORT_PROD}:${APP_PORT} \
              -e NODE_ENV=production \
              -e PORT=${APP_PORT} \
              ${IMAGE}
    ...
    stage('Monitor and Alert stage') {
      steps {
        sh '''#!/bin/bash
          set -euo pipefail
          echo "[Monitoring] Verify PROD from inside container"
